...
module nft_protocol::pantagruel {
    use std::vector;
    ...
    struct Pantagruel has key, store {
        id: UID,
    ...
    public entry fun deposit_archetype<C>(
        warehouse: &mut Pantagruel,
        ...
        let item = NftItem {
            archetype_id,
            supply: flyweight::supply(&archetype),
        };

        vector::push_back(&mut warehouse.nfts_on_sale, item);

        dof::add(&mut warehouse.id, archetype_id, archetype);
    }
    ...
    public fun length(warehouse: &Pantagruel): u64 {
        vector::length(&warehouse.nfts_on_sale)
    }

    public fun is_empty(warehouse: &Pantagruel): bool {
